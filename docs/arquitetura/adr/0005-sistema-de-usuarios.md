...
Cargos do usuário (`user_role`) são gerenciados por endpoints próprios (`POST`/`GET`/`DELETE /users/:id/roles`), espelhando o padrão de `role_permission` do ADR 0004 §6. Só são aceitos cargos da **empresa da sessão** (cross-tenant → 4xx); o vínculo é sempre da empresa da sessão. `DELETE /users/:id/roles/:roleId` rejeita remover o cargo `is_admin` do **último usuário com cargo `is_admin` ativo** da empresa → **409** (mesma invariante da seção 4).

**Um cargo por pessoa/empresa** (emenda): o usuário tem **no máximo um cargo por empresa** — o unique passa a ser `(company_id, user_id)` (migration `0009`, que também deduplica linhas multi-cargo: prioridade para `is_admin`, senão a mais antiga). Atribuir um segundo cargo (mesmo diferente) → **409** (`'Usuário já possui um cargo nesta empresa.'`). Além dos endpoints próprios, o `roleId` é aceito em `POST /users` (cria pessoa/vínculo já com cargo, na mesma transação) e em `PATCH /users/:id` (troca o cargo — replace do cargo único; `roleId: null` remove o cargo). A listagem e o detalhe de usuários devolvem o **resumo do cargo** (`role: { userRoleId, roleId, roleName, isAdmin } | null`), enriquecido em lote na listagem (sem N+1).

**Governança**: o cargo `is_admin` **é do sistema** — não é editável por nenhum usuário (criar/editar/desativar/reativar/excluir o cargo já é proibido pelo ADR 0004 §4); dele se gerencia **apenas a atribuição** (`user_role`). Atribuir **ou retirar** um cargo `is_admin` de um usuário exige que o **ator tenha cargo `is_admin` ativo** na empresa da sessão → **403** (um gestor com `MANAGE_USERS` sem `is_admin` não cria nem remove administradores). Gerenciar cargos de um usuário com cargo `is_admin` ativo também exige ator `is_admin` → **403**.
