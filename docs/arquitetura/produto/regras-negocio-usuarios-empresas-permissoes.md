...
> Decisões arquiteturais de referência: [ADR 0001 — Migrations e seeds iniciais](../adr/0001-migrations-seeds-iniciais.md) e [ADR 0002 — A pessoa é a identidade e a empresa é um vínculo](../adr/0002-a-pessoa-e-a-identidade-e-a-empresa-e-um-vinculo.md).
...
20. `permission` é um **catálogo global** do sistema (sem `company_id`); o vínculo com a empresa ocorre via `role_permission`.
21. **Cargos (`role`)** são por empresa; cada cargo pode ser marcado como `is_admin` (acesso total à administração).
22. **Permissões são granulares** (`role_permission`): um cargo possui um conjunto de permissões; um usuário pode ter vários cargos (`user_role`).
23. **Escalabilidade não engessada**: a administração pode **mudar cargos, permissões e a visualização de seus usuários** — o mapeamento inicial é um ponto de partida, não um contrato rígido.
24. **Acesso por papel**: os papéis são **Porteiro**, **Segurança**, **Administração** e **Presidência** — cada um com permissões específicas.
25. No código, permissões são referenciadas pelo enum `PermissionCode` (nunca strings hardcoded), aplicadas via `JwtAuthGuard` + `PermissionsGuard`.
26. **Papéis nunca vazam entre empresas**: `user_role`/`role_permission` já são escopados por `company_id`; a resolução usa sempre `(user_id, companyId)`.

### Cargos seedados (empresa padrão SOMAR)

27. **Administração** (`is_admin = true`): todas as 23 permissões.
28. **Segurança**: tudo do porteiro + `MANAGE_BLOCKS`.
29. **Presidência**: `VIEW_DASHBOARDS`, `GRANT_FREE_PASS`, `MANAGE_BLOCKS`.
30. **Porteiro**: `REGISTER_ENTRY`, `REGISTER_EXIT`, `REGISTER_DENIAL`, `CREATE_ACCESS_REQUEST`, `CANCEL_ACCESS_REQUEST`, `CREATE_BLOCK_REQUEST`, `VIEW_DASHBOARDS`.

## 5. Device (app do porteiro)

31. O tablet da portaria é **compartilhado** (sem dono) — vários porteiros logam no mesmo `device`; quem executa cada ação é registrado em `doorman_id`/`requested_by`.
32. **Registro de device**: na 1ª execução o app se registra (`name`, `token` único, `platform`, `app_version`) — usado no sync e na auditoria.
33. O tablet pode ser vinculado a uma **portaria** (`device.entrance_id`), preenchendo `entrance_id` dos eventos automaticamente.
34. `last_sync_at` é usado no **pull incremental**; device **desativado** (`is_active = false`) limpa o cache do app.
35. **Offline**: cache local com **minimização de dados pessoais (LGPD)** — no dispositivo fica apenas o **nome** do motorista; documento, telefone e foto apenas **online**. Retenção do cache: limpar ao deslogar, ao desativar o device e após **30 dias sem sincronizar**.
36. **Sync**: fila local (outbox) com `idempotency_key`; o servidor **revalida as regras** de negócio de cada item e faz o pull incremental desde `last_sync_at`. O servidor é a **fonte da verdade**.

## 6. Importação de planilhas

37. A importação (Excel/xlsx) ocorre por **jobs** (`import_job`), com fila e status `PENDING`/`PROCESSING`/`DONE`/`FAILED`/`PARTIAL` (erros por linha em `errors`).
38. Tipos de importação: `VEHICLE`, `USER`, `USER_VEHICLE`.
39. **Pós-importação**: a aplicação **web** gera os QR codes dos veículos importados (**em lote**) e disponibiliza a impressão.
40. Importação é uma feature do MVP — cadastros em massa para facilitar a adoção inicial.
...
44. **Super admin** de múltiplas autarquias (gestão de autarquias) — versão completa.
45. **Integração com Gmail/Google** para avisos à administração e disparo de relatórios — versão completa.
46. **Recuperação de senha** multi-empresa (redefinição que vale para todos os vínculos do mesmo e-mail) — a decidir quando entrar no escopo.
