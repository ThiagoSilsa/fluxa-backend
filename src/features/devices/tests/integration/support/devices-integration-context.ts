...
/**
 * IDs e credenciais usados pelos testes de integração da feature `devices`
 * (seeds 0001–0002 + helpers de seed criados aqui).
 */
export const DEVICES_SEEDED = {
  ...
  /** Cargo `Porteiro` (sem MANAGE_DEVICES). */
  PORTEIRO_ROLE_ID: '20000000-0000-0000-0000-000000000004',
} as const;

/** Contexto de integração da feature `devices`. */
export interface DevicesIntegrationContext {
...
/**
 * Monta o app de integração da feature `devices` — Testcontainers (Postgres
 * real) + migrations/seeds do zero + `AppModule` (com `DevicesModule`) + HTTP
 * via supertest.
 *
 * Espelha `createEntrancesIntegrationContext`: valida mapeamento ORM, guards,
 * `ValidationPipe` e as rotas de `/devices`.
 */
export async function createDevicesIntegrationContext(): Promise<DevicesIntegrationContext> {
 ...
 * SOMAR — para cenários de autorização (ex.: Porteiro sem MANAGE_DEVICES).
 *
...
async function seedUserWithRole(
  ...
  await dataSource.query(
    ...
    [userId, DEVICES_SEEDED.SOMAR_COMPANY_ID],
  );
  ...
  await dataSource.query(
    ...
    [DEVICES_SEEDED.SOMAR_COMPANY_ID, userId, roleId],
  );
}
