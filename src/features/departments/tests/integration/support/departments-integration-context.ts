 ...
 * `departments` (seeds 0001–0002 + helpers de seed criados aqui).
 */
export const DEPARTMENTS_SEEDED = {
  ...
  /** Cargo `Porteiro` (sem MANAGE_DEPARTMENTS). */
  PORTEIRO_ROLE_ID: '20000000-0000-0000-0000-000000000004',
} as const;

/** Contexto de integração da feature `departments`. */
export interface DepartmentsIntegrationContext {
...
/**
 * Monta o app de integração da feature `departments` — Testcontainers
 * (Postgres real) + migrations/seeds do zero + `AppModule` (com
 * `DepartmentsModule`) + HTTP via supertest.
 *
 * Espelha `createRolesIntegrationContext`: valida mapeamento ORM, guards,
 * `ValidationPipe` e as rotas de `/departments`.
 */
export async function createDepartmentsIntegrationContext(): Promise<DepartmentsIntegrationContext> {
...
async function seedUserWithRole(
  ...
  await dataSource.query(
    ...
    [userId, DEPARTMENTS_SEEDED.SOMAR_COMPANY_ID],
  );
  ...
  await dataSource.query(
    ...
    [DEPARTMENTS_SEEDED.SOMAR_COMPANY_ID, userId, roleId],
  );
}
