...
describe('ListUserOptionsUseCase', () => {
  let useCase: ListUserOptionsUseCase;
  ...
  beforeEach(async () => {
    ...
    useCase = module.get(ListUserOptionsUseCase);
  });

  it('lista opções enxutas { id, name, email } apenas de vínculos ativos', async () => {
    ...
    const result = await useCase.execute(
      actor,
      new ListUserOptionsInputDto(undefined, 20, 0),
    );
  ...
  it('repassa busca e paginação para o repositório', async () => {
    ...
    await useCase.execute(actor, new ListUserOptionsInputDto('joa', 10, 20));
