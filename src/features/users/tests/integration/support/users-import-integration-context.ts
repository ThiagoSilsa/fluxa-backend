 ...
 * usuários.
 */
export const USERS_IMPORT_SEEDED = {
  ...
  /** Cargo `Porteiro` (seed — nome 'Porteiro'). */
  PORTEIRO_ROLE_ID: '20000000-0000-0000-0000-000000000004',
  /** Senha default de onboarding (fallback do ConfigService). */
  DEFAULT_PASSWORD: 'Somar@123',
} as const;
...
/** Contexto de integração do importador de usuários. */
export interface UsersImportIntegrationContext {
...
/**
 * Monta o app de integração do importador de usuários — Postgres e Redis via
 * Testcontainers + migrations/seeds do zero + `AppModule` + HTTP.
 */
export async function createUsersImportIntegrationContext(): Promise<UsersImportIntegrationContext> {
  ...
  process.env.IMPORT_DEFAULT_PASSWORD = USERS_IMPORT_SEEDED.DEFAULT_PASSWORD;
