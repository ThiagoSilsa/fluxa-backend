...
import type { ImportUsersResult } from '../dto/import-users-result';

// Types
import type { AuthenticatedUserEntity } from '../../../auth/domain/entities/authenticated-user.entity';

/**
 * Colunas obrigatórias na planilha de usuários (aba `data`).
 */
const REQUIRED_COLUMNS = ['email', 'name'];

/**
 * Colunas aceitas na planilha de usuários.
 */
...
/**
 * Use case de upload de importação de usuários (ADR 0007 §5/§6/§8).
 *
 ...
 * trabalho pesado fica no `ImportUsersProcessor`.
 */
@Injectable()
export class ImportUsersUseCase {
  ...
  constructor(
    ...
    @InjectQueue(QUEUE_NAMES.IMPORT_USERS)
  ...
  public async execute(
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'users-import-'));
    ...
    try {
      ...
      const jobData: ImportUsersJobData = {
        jobId: job.id,
        companyId,
        createdByUserId: actor.id,
