...
import type { ImportUserVehiclesResult } from '../dto/import-user-vehicles-result';

// Types
import type { AuthenticatedUserEntity } from '../../../auth/domain/entities/authenticated-user.entity';

/**
 * Colunas obrigatórias na planilha de vínculo usuário-veículo (aba `data`).
 */
const REQUIRED_COLUMNS = ['vehiclePlate', 'userEmail'];

/**
 * Colunas aceitas na planilha de vínculo usuário-veículo.
 */
const EXPECTED_COLUMNS = ['vehiclePlate', 'userEmail', 'isPrimary', 'canDrive'];

/**
 * Use case de upload de importação de vínculo usuário-veículo
 * (ADR 0007 §5/§6/§8).
 *
 ...
 * trabalho pesado fica no `ImportUserVehiclesProcessor`.
 */
@Injectable()
export class ImportUserVehiclesUseCase {
  private readonly logger = new Logger(ImportUserVehiclesUseCase.name);

  constructor(
    @Inject(IMPORT_JOB_REPOSITORY)
    private readonly importJobRepository: ImportJobRepository,
    @InjectQueue(QUEUE_NAMES.IMPORT_USER_VEHICLES)
  ...
  public async execute(
    ...
    const tempDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'user-vehicles-import-'),
    );
    ...
    try {
      ...
      const jobData: ImportUserVehiclesJobData = {
        jobId: job.id,
        companyId,
        createdByUserId: actor.id,
