...
describe('GetVehicleQrUseCase', () => {
  let useCase: GetVehicleQrUseCase;

  const vehicleQrRepoMock = {
    findActiveByVehicleIdAndCompanyId: jest.fn(),
  } as jest.Mocked<
    Pick<VehicleQrRepository, 'findActiveByVehicleIdAndCompanyId'>
  >;
  ...
  beforeEach(async () => {
    ...
    useCase = module.get(GetVehicleQrUseCase);
  });

  it('devolve o QR ativo (mesmo code) para reimpressão', async () => {
  ...
  it('lança NotFoundException quando não há QR ativo (nunca emitido/revogado)', async () => {
    vehicleRepoMock.findByIdAndCompanyId.mockResolvedValue(vehicle);
