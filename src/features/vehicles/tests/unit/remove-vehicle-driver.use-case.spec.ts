...
describe('RemoveVehicleDriverUseCase', () => {
  let useCase: RemoveVehicleDriverUseCase;
  ...
  const userVehicleRepoMock = {
    ...
    removeByIdAndCompanyId: jest.fn(),
  } as jest.Mocked<
    Pick<
      UserVehicleRepository,
      'findByUserIdAndVehicleIdAndCompanyId' | 'removeByIdAndCompanyId'
    >
  >;
  ...
  beforeEach(async () => {
    ...
    useCase = module.get(RemoveVehicleDriverUseCase);
  });

  it('remove o vínculo fisicamente (delete físico)', async () => {
    ...
    await useCase.execute(actor, new RemoveDriverInputDto(vehicleId, userId));

    expect(userVehicleRepoMock.removeByIdAndCompanyId).toHaveBeenCalledWith(
      link.id,
      actor.companyId,
    );
  });
  ...
  it('lança NotFoundException quando o veículo não existe na empresa', async () => {
    ...
    await expect(
      useCase.execute(actor, new RemoveDriverInputDto(vehicleId, userId)),
    ).rejects.toThrow(NotFoundException);
    expect(userVehicleRepoMock.removeByIdAndCompanyId).not.toHaveBeenCalled();
  });
