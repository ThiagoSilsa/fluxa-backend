...
describe('RevokeVehicleQrUseCase', () => {
  let useCase: RevokeVehicleQrUseCase;
  ...
  const vehicleQrRepoMock = {
    ...
    Pick<
      VehicleQrRepository,
      'findActiveByVehicleIdAndCompanyId' | 'deactivateByIdAndCompanyId'
    >
  >;
  ...
  beforeEach(async () => {
    ...
    useCase = module.get(RevokeVehicleQrUseCase);
  });

  it('revoga o QR ativo (desativa sem criar outro)', async () => {
    ...
    vehicleQrRepoMock.deactivateByIdAndCompanyId.mockResolvedValue(true);

    await useCase.execute(actor, new GetVehicleQrInputDto(vehicle.id));

    expect(vehicleQrRepoMock.deactivateByIdAndCompanyId).toHaveBeenCalledWith(
      activeQr.id,
      actor.companyId,
    );
  });
  ...
  it('lança NotFoundException quando o veículo não existe na empresa', async () => {
    ...
    expect(vehicleQrRepoMock.deactivateByIdAndCompanyId).not.toHaveBeenCalled();
  });

  it('lança ConflictException quando não há QR ativo para revogar', async () => {
    vehicleRepoMock.findByIdAndCompanyId.mockResolvedValue(vehicle);
